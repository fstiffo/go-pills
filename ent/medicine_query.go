// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"
	"database/sql/driver"
	"fmt"
	"fstiffo/pills/ent/activeingredient"
	"fstiffo/pills/ent/medicine"
	"fstiffo/pills/ent/predicate"
	"fstiffo/pills/ent/purchase"
	"fstiffo/pills/ent/stockinglog"
	"math"

	"entgo.io/ent"
	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
)

// MedicineQuery is the builder for querying Medicine entities.
type MedicineQuery struct {
	config
	ctx                  *QueryContext
	order                []medicine.OrderOption
	inters               []Interceptor
	predicates           []predicate.Medicine
	withPurchases        *PurchaseQuery
	withStockingLogs     *StockingLogQuery
	withActiveIngredient *ActiveIngredientQuery
	withFKs              bool
	// intermediate query (i.e. traversal path).
	sql  *sql.Selector
	path func(context.Context) (*sql.Selector, error)
}

// Where adds a new predicate for the MedicineQuery builder.
func (mq *MedicineQuery) Where(ps ...predicate.Medicine) *MedicineQuery {
	mq.predicates = append(mq.predicates, ps...)
	return mq
}

// Limit the number of records to be returned by this query.
func (mq *MedicineQuery) Limit(limit int) *MedicineQuery {
	mq.ctx.Limit = &limit
	return mq
}

// Offset to start from.
func (mq *MedicineQuery) Offset(offset int) *MedicineQuery {
	mq.ctx.Offset = &offset
	return mq
}

// Unique configures the query builder to filter duplicate records on query.
// By default, unique is set to true, and can be disabled using this method.
func (mq *MedicineQuery) Unique(unique bool) *MedicineQuery {
	mq.ctx.Unique = &unique
	return mq
}

// Order specifies how the records should be ordered.
func (mq *MedicineQuery) Order(o ...medicine.OrderOption) *MedicineQuery {
	mq.order = append(mq.order, o...)
	return mq
}

// QueryPurchases chains the current query on the "purchases" edge.
func (mq *MedicineQuery) QueryPurchases() *PurchaseQuery {
	query := (&PurchaseClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := mq.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(medicine.Table, medicine.FieldID, selector),
			sqlgraph.To(purchase.Table, purchase.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, medicine.PurchasesTable, medicine.PurchasesColumn),
		)
		fromU = sqlgraph.SetNeighbors(mq.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// QueryStockingLogs chains the current query on the "stocking_logs" edge.
func (mq *MedicineQuery) QueryStockingLogs() *StockingLogQuery {
	query := (&StockingLogClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := mq.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(medicine.Table, medicine.FieldID, selector),
			sqlgraph.To(stockinglog.Table, stockinglog.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, medicine.StockingLogsTable, medicine.StockingLogsColumn),
		)
		fromU = sqlgraph.SetNeighbors(mq.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// QueryActiveIngredient chains the current query on the "active_ingredient" edge.
func (mq *MedicineQuery) QueryActiveIngredient() *ActiveIngredientQuery {
	query := (&ActiveIngredientClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := mq.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(medicine.Table, medicine.FieldID, selector),
			sqlgraph.To(activeingredient.Table, activeingredient.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, medicine.ActiveIngredientTable, medicine.ActiveIngredientColumn),
		)
		fromU = sqlgraph.SetNeighbors(mq.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// First returns the first Medicine entity from the query.
// Returns a *NotFoundError when no Medicine was found.
func (mq *MedicineQuery) First(ctx context.Context) (*Medicine, error) {
	nodes, err := mq.Limit(1).All(setContextOp(ctx, mq.ctx, ent.OpQueryFirst))
	if err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
		return nil, &NotFoundError{medicine.Label}
	}
	return nodes[0], nil
}

// FirstX is like First, but panics if an error occurs.
func (mq *MedicineQuery) FirstX(ctx context.Context) *Medicine {
	node, err := mq.First(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return node
}

// FirstID returns the first Medicine ID from the query.
// Returns a *NotFoundError when no Medicine ID was found.
func (mq *MedicineQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
	if ids, err = mq.Limit(1).IDs(setContextOp(ctx, mq.ctx, ent.OpQueryFirstID)); err != nil {
		return
	}
	if len(ids) == 0 {
		err = &NotFoundError{medicine.Label}
		return
	}
	return ids[0], nil
}

// FirstIDX is like FirstID, but panics if an error occurs.
func (mq *MedicineQuery) FirstIDX(ctx context.Context) int {
	id, err := mq.FirstID(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return id
}

// Only returns a single Medicine entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one Medicine entity is found.
// Returns a *NotFoundError when no Medicine entities are found.
func (mq *MedicineQuery) Only(ctx context.Context) (*Medicine, error) {
	nodes, err := mq.Limit(2).All(setContextOp(ctx, mq.ctx, ent.OpQueryOnly))
	if err != nil {
		return nil, err
	}
	switch len(nodes) {
	case 1:
		return nodes[0], nil
	case 0:
		return nil, &NotFoundError{medicine.Label}
	default:
		return nil, &NotSingularError{medicine.Label}
	}
}

// OnlyX is like Only, but panics if an error occurs.
func (mq *MedicineQuery) OnlyX(ctx context.Context) *Medicine {
	node, err := mq.Only(ctx)
	if err != nil {
		panic(err)
	}
	return node
}

// OnlyID is like Only, but returns the only Medicine ID in the query.
// Returns a *NotSingularError when more than one Medicine ID is found.
// Returns a *NotFoundError when no entities are found.
func (mq *MedicineQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
	if ids, err = mq.Limit(2).IDs(setContextOp(ctx, mq.ctx, ent.OpQueryOnlyID)); err != nil {
		return
	}
	switch len(ids) {
	case 1:
		id = ids[0]
	case 0:
		err = &NotFoundError{medicine.Label}
	default:
		err = &NotSingularError{medicine.Label}
	}
	return
}

// OnlyIDX is like OnlyID, but panics if an error occurs.
func (mq *MedicineQuery) OnlyIDX(ctx context.Context) int {
	id, err := mq.OnlyID(ctx)
	if err != nil {
		panic(err)
	}
	return id
}

// All executes the query and returns a list of Medicines.
func (mq *MedicineQuery) All(ctx context.Context) ([]*Medicine, error) {
	ctx = setContextOp(ctx, mq.ctx, ent.OpQueryAll)
	if err := mq.prepareQuery(ctx); err != nil {
		return nil, err
	}
	qr := querierAll[[]*Medicine, *MedicineQuery]()
	return withInterceptors[[]*Medicine](ctx, mq, qr, mq.inters)
}

// AllX is like All, but panics if an error occurs.
func (mq *MedicineQuery) AllX(ctx context.Context) []*Medicine {
	nodes, err := mq.All(ctx)
	if err != nil {
		panic(err)
	}
	return nodes
}

// IDs executes the query and returns a list of Medicine IDs.
func (mq *MedicineQuery) IDs(ctx context.Context) (ids []int, err error) {
	if mq.ctx.Unique == nil && mq.path != nil {
		mq.Unique(true)
	}
	ctx = setContextOp(ctx, mq.ctx, ent.OpQueryIDs)
	if err = mq.Select(medicine.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}

// IDsX is like IDs, but panics if an error occurs.
func (mq *MedicineQuery) IDsX(ctx context.Context) []int {
	ids, err := mq.IDs(ctx)
	if err != nil {
		panic(err)
	}
	return ids
}

// Count returns the count of the given query.
func (mq *MedicineQuery) Count(ctx context.Context) (int, error) {
	ctx = setContextOp(ctx, mq.ctx, ent.OpQueryCount)
	if err := mq.prepareQuery(ctx); err != nil {
		return 0, err
	}
	return withInterceptors[int](ctx, mq, querierCount[*MedicineQuery](), mq.inters)
}

// CountX is like Count, but panics if an error occurs.
func (mq *MedicineQuery) CountX(ctx context.Context) int {
	count, err := mq.Count(ctx)
	if err != nil {
		panic(err)
	}
	return count
}

// Exist returns true if the query has elements in the graph.
func (mq *MedicineQuery) Exist(ctx context.Context) (bool, error) {
	ctx = setContextOp(ctx, mq.ctx, ent.OpQueryExist)
	switch _, err := mq.FirstID(ctx); {
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("ent: check existence: %w", err)
	default:
		return true, nil
	}
}

// ExistX is like Exist, but panics if an error occurs.
func (mq *MedicineQuery) ExistX(ctx context.Context) bool {
	exist, err := mq.Exist(ctx)
	if err != nil {
		panic(err)
	}
	return exist
}

// Clone returns a duplicate of the MedicineQuery builder, including all associated steps. It can be
// used to prepare common query builders and use them differently after the clone is made.
func (mq *MedicineQuery) Clone() *MedicineQuery {
	if mq == nil {
		return nil
	}
	return &MedicineQuery{
		config:               mq.config,
		ctx:                  mq.ctx.Clone(),
		order:                append([]medicine.OrderOption{}, mq.order...),
		inters:               append([]Interceptor{}, mq.inters...),
		predicates:           append([]predicate.Medicine{}, mq.predicates...),
		withPurchases:        mq.withPurchases.Clone(),
		withStockingLogs:     mq.withStockingLogs.Clone(),
		withActiveIngredient: mq.withActiveIngredient.Clone(),
		// clone intermediate query.
		sql:  mq.sql.Clone(),
		path: mq.path,
	}
}

// WithPurchases tells the query-builder to eager-load the nodes that are connected to
// the "purchases" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MedicineQuery) WithPurchases(opts ...func(*PurchaseQuery)) *MedicineQuery {
	query := (&PurchaseClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withPurchases = query
	return mq
}

// WithStockingLogs tells the query-builder to eager-load the nodes that are connected to
// the "stocking_logs" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MedicineQuery) WithStockingLogs(opts ...func(*StockingLogQuery)) *MedicineQuery {
	query := (&StockingLogClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withStockingLogs = query
	return mq
}

// WithActiveIngredient tells the query-builder to eager-load the nodes that are connected to
// the "active_ingredient" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MedicineQuery) WithActiveIngredient(opts ...func(*ActiveIngredientQuery)) *MedicineQuery {
	query := (&ActiveIngredientClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withActiveIngredient = query
	return mq
}

// GroupBy is used to group vertices by one or more fields/columns.
// It is often used with aggregate functions, like: count, max, mean, min, sum.
//
// Example:
//
//	var v []struct {
//		Name string `json:"name,omitempty"`
//		Count int `json:"count,omitempty"`
//	}
//
//	client.Medicine.Query().
//		GroupBy(medicine.FieldName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (mq *MedicineQuery) GroupBy(field string, fields ...string) *MedicineGroupBy {
	mq.ctx.Fields = append([]string{field}, fields...)
	grbuild := &MedicineGroupBy{build: mq}
	grbuild.flds = &mq.ctx.Fields
	grbuild.label = medicine.Label
	grbuild.scan = grbuild.Scan
	return grbuild
}

// Select allows the selection one or more fields/columns for the given query,
// instead of selecting all fields in the entity.
//
// Example:
//
//	var v []struct {
//		Name string `json:"name,omitempty"`
//	}
//
//	client.Medicine.Query().
//		Select(medicine.FieldName).
//		Scan(ctx, &v)
func (mq *MedicineQuery) Select(fields ...string) *MedicineSelect {
	mq.ctx.Fields = append(mq.ctx.Fields, fields...)
	sbuild := &MedicineSelect{MedicineQuery: mq}
	sbuild.label = medicine.Label
	sbuild.flds, sbuild.scan = &mq.ctx.Fields, sbuild.Scan
	return sbuild
}

// Aggregate returns a MedicineSelect configured with the given aggregations.
func (mq *MedicineQuery) Aggregate(fns ...AggregateFunc) *MedicineSelect {
	return mq.Select().Aggregate(fns...)
}

func (mq *MedicineQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range mq.inters {
		if inter == nil {
			return fmt.Errorf("ent: uninitialized interceptor (forgotten import ent/runtime?)")
		}
		if trv, ok := inter.(Traverser); ok {
			if err := trv.Traverse(ctx, mq); err != nil {
				return err
			}
		}
	}
	for _, f := range mq.ctx.Fields {
		if !medicine.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("ent: invalid field %q for query", f)}
		}
	}
	if mq.path != nil {
		prev, err := mq.path(ctx)
		if err != nil {
			return err
		}
		mq.sql = prev
	}
	return nil
}

func (mq *MedicineQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Medicine, error) {
	var (
		nodes       = []*Medicine{}
		withFKs     = mq.withFKs
		_spec       = mq.querySpec()
		loadedTypes = [3]bool{
			mq.withPurchases != nil,
			mq.withStockingLogs != nil,
			mq.withActiveIngredient != nil,
		}
	)
	if mq.withActiveIngredient != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, medicine.ForeignKeys...)
	}
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Medicine).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &Medicine{config: mq.config}
		nodes = append(nodes, node)
		node.Edges.loadedTypes = loadedTypes
		return node.assignValues(columns, values)
	}
	for i := range hooks {
		hooks[i](ctx, _spec)
	}
	if err := sqlgraph.QueryNodes(ctx, mq.driver, _spec); err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
		return nodes, nil
	}
	if query := mq.withPurchases; query != nil {
		if err := mq.loadPurchases(ctx, query, nodes,
			func(n *Medicine) { n.Edges.Purchases = []*Purchase{} },
			func(n *Medicine, e *Purchase) { n.Edges.Purchases = append(n.Edges.Purchases, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withStockingLogs; query != nil {
		if err := mq.loadStockingLogs(ctx, query, nodes,
			func(n *Medicine) { n.Edges.StockingLogs = []*StockingLog{} },
			func(n *Medicine, e *StockingLog) { n.Edges.StockingLogs = append(n.Edges.StockingLogs, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withActiveIngredient; query != nil {
		if err := mq.loadActiveIngredient(ctx, query, nodes, nil,
			func(n *Medicine, e *ActiveIngredient) { n.Edges.ActiveIngredient = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

func (mq *MedicineQuery) loadPurchases(ctx context.Context, query *PurchaseQuery, nodes []*Medicine, init func(*Medicine), assign func(*Medicine, *Purchase)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Medicine)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
		if init != nil {
			init(nodes[i])
		}
	}
	query.withFKs = true
	query.Where(predicate.Purchase(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(medicine.PurchasesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		fk := n.medicine_purchases
		if fk == nil {
			return fmt.Errorf(`foreign-key "medicine_purchases" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "medicine_purchases" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (mq *MedicineQuery) loadStockingLogs(ctx context.Context, query *StockingLogQuery, nodes []*Medicine, init func(*Medicine), assign func(*Medicine, *StockingLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Medicine)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
		if init != nil {
			init(nodes[i])
		}
	}
	query.withFKs = true
	query.Where(predicate.StockingLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(medicine.StockingLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		fk := n.medicine_stocking_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "medicine_stocking_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "medicine_stocking_logs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (mq *MedicineQuery) loadActiveIngredient(ctx context.Context, query *ActiveIngredientQuery, nodes []*Medicine, init func(*Medicine), assign func(*Medicine, *ActiveIngredient)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Medicine)
	for i := range nodes {
		if nodes[i].active_ingredient_medicines == nil {
			continue
		}
		fk := *nodes[i].active_ingredient_medicines
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	if len(ids) == 0 {
		return nil
	}
	query.Where(activeingredient.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "active_ingredient_medicines" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
	}
	return nil
}

func (mq *MedicineQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := mq.querySpec()
	_spec.Node.Columns = mq.ctx.Fields
	if len(mq.ctx.Fields) > 0 {
		_spec.Unique = mq.ctx.Unique != nil && *mq.ctx.Unique
	}
	return sqlgraph.CountNodes(ctx, mq.driver, _spec)
}

func (mq *MedicineQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(medicine.Table, medicine.Columns, sqlgraph.NewFieldSpec(medicine.FieldID, field.TypeInt))
	_spec.From = mq.sql
	if unique := mq.ctx.Unique; unique != nil {
		_spec.Unique = *unique
	} else if mq.path != nil {
		_spec.Unique = true
	}
	if fields := mq.ctx.Fields; len(fields) > 0 {
		_spec.Node.Columns = make([]string, 0, len(fields))
		_spec.Node.Columns = append(_spec.Node.Columns, medicine.FieldID)
		for i := range fields {
			if fields[i] != medicine.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, fields[i])
			}
		}
	}
	if ps := mq.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if limit := mq.ctx.Limit; limit != nil {
		_spec.Limit = *limit
	}
	if offset := mq.ctx.Offset; offset != nil {
		_spec.Offset = *offset
	}
	if ps := mq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	return _spec
}

func (mq *MedicineQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(mq.driver.Dialect())
	t1 := builder.Table(medicine.Table)
	columns := mq.ctx.Fields
	if len(columns) == 0 {
		columns = medicine.Columns
	}
	selector := builder.Select(t1.Columns(columns...)...).From(t1)
	if mq.sql != nil {
		selector = mq.sql
		selector.Select(selector.Columns(columns...)...)
	}
	if mq.ctx.Unique != nil && *mq.ctx.Unique {
		selector.Distinct()
	}
	for _, p := range mq.predicates {
		p(selector)
	}
	for _, p := range mq.order {
		p(selector)
	}
	if offset := mq.ctx.Offset; offset != nil {
		// limit is mandatory for offset clause. We start
		// with default value, and override it below if needed.
		selector.Offset(*offset).Limit(math.MaxInt32)
	}
	if limit := mq.ctx.Limit; limit != nil {
		selector.Limit(*limit)
	}
	return selector
}

// MedicineGroupBy is the group-by builder for Medicine entities.
type MedicineGroupBy struct {
	selector
	build *MedicineQuery
}

// Aggregate adds the given aggregation functions to the group-by query.
func (mgb *MedicineGroupBy) Aggregate(fns ...AggregateFunc) *MedicineGroupBy {
	mgb.fns = append(mgb.fns, fns...)
	return mgb
}

// Scan applies the selector query and scans the result into the given value.
func (mgb *MedicineGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, mgb.build.ctx, ent.OpQueryGroupBy)
	if err := mgb.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*MedicineQuery, *MedicineGroupBy](ctx, mgb.build, mgb, mgb.build.inters, v)
}

func (mgb *MedicineGroupBy) sqlScan(ctx context.Context, root *MedicineQuery, v any) error {
	selector := root.sqlQuery(ctx).Select()
	aggregation := make([]string, 0, len(mgb.fns))
	for _, fn := range mgb.fns {
		aggregation = append(aggregation, fn(selector))
	}
	if len(selector.SelectedColumns()) == 0 {
		columns := make([]string, 0, len(*mgb.flds)+len(mgb.fns))
		for _, f := range *mgb.flds {
			columns = append(columns, selector.C(f))
		}
		columns = append(columns, aggregation...)
		selector.Select(columns...)
	}
	selector.GroupBy(selector.Columns(*mgb.flds...)...)
	if err := selector.Err(); err != nil {
		return err
	}
	rows := &sql.Rows{}
	query, args := selector.Query()
	if err := mgb.build.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
	return sql.ScanSlice(rows, v)
}

// MedicineSelect is the builder for selecting fields of Medicine entities.
type MedicineSelect struct {
	*MedicineQuery
	selector
}

// Aggregate adds the given aggregation functions to the selector query.
func (ms *MedicineSelect) Aggregate(fns ...AggregateFunc) *MedicineSelect {
	ms.fns = append(ms.fns, fns...)
	return ms
}

// Scan applies the selector query and scans the result into the given value.
func (ms *MedicineSelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, ms.ctx, ent.OpQuerySelect)
	if err := ms.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*MedicineQuery, *MedicineSelect](ctx, ms.MedicineQuery, ms, ms.inters, v)
}

func (ms *MedicineSelect) sqlScan(ctx context.Context, root *MedicineQuery, v any) error {
	selector := root.sqlQuery(ctx)
	aggregation := make([]string, 0, len(ms.fns))
	for _, fn := range ms.fns {
		aggregation = append(aggregation, fn(selector))
	}
	switch n := len(*ms.selector.flds); {
	case n == 0 && len(aggregation) > 0:
		selector.Select(aggregation...)
	case n != 0 && len(aggregation) > 0:
		selector.AppendSelect(aggregation...)
	}
	rows := &sql.Rows{}
	query, args := selector.Query()
	if err := ms.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
	return sql.ScanSlice(rows, v)
}
