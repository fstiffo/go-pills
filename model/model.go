...
type ActiveIngredient struct {
	...
	ATC  string `gorm:"index:unique;not null;size:7;check:length(ATC) = 7"`
	// Stock stores units of active principle x 10 (e.g. 1 mg = 10)
...
type Medicine struct {
	...
	// Dosage stores units of active ingredient for each unit of medicine x 10 (e.g. 1 mg = 10)
...
type Prescription struct {
	gorm.Model
	RelatedATC string `gorm:"index;not null"`
	// Dosage store active ingredient units prescibed x 10 (e.g. 1 mg = 10)
	Dosage          int64 `gorm:"not null;check:dosage > 0"`
...
type ConsumptionLog struct {
	gorm.Model
	PrescriptionID uint      `index;gorm:"not null"`
	RelatedATC     string    `index;gorm:"not null"`
	ConsumedAt     time.Time `gorm:"not null;default:CURRENT_TIMESTAMP"`
	// Units stores active ingredient units comsumed x 10 (e.g. 1 mg = 10)
